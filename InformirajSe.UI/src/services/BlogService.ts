...
export class BlogService{
    ...
    public async addNewBlog(title: string, bodyContent: string){
        ...
        try {
            const response = await fetch('http://localhost:8080/api/new-blog', {
                method: 'POST',
                credentials: 'include',
                headers: {
    ...
    public async editBlog(blogId: string, title: string, bodyContent: string, callback: Function){
        ...
        try {
            const response = await fetch(`http://localhost:8080/api/edit-blog`, {
                method: 'POST',
                credentials: 'include',
                headers: {
                    "Content-Type": "application/json"
    ...
    public async deleteBlog(blogId: string){
        try {
            const response = await fetch('http://localhost:8080/api/delete-blog/'+ blogId, {
                method: 'POST',
                credentials: 'include',
                headers: {
                    "Content-Type": "application/json"
    ...
    public async getMostInteresting(): Promise<Blog[] | undefined> {
        try {
            const response = await fetch('http://localhost:8080/api/most-interesting', {
                method: 'GET',
                credentials: 'include'
            });
